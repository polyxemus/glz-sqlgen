...
TEST(DateFunctionsTest, YearColumn) {
    auto query = select_from<Events>(year("event_date"_col));
...
TEST(DateFunctionsTest, YearInWhere) {
    auto query = select_from<Events>()
        | where(year("event_date"_col) == 2024_v);
...
TEST(DateFunctionsTest, MonthColumn) {
    auto query = select_from<Events>(month("event_date"_col));
...
TEST(DateFunctionsTest, MonthInWhere) {
    auto query = select_from<Events>()
        | where(month("event_date"_col) == 12_v);
...
TEST(DateFunctionsTest, MonthRange) {
    auto query = select_from<Events>()
        | where(month("event_date"_col) >= 6_v && month("event_date"_col) <= 8_v);
...
TEST(DateFunctionsTest, DayColumn) {
    auto query = select_from<Events>(day("event_date"_col));
...
TEST(DateFunctionsTest, DayInWhere) {
    auto query = select_from<Events>()
        | where(day("event_date"_col) == 15_v);
...
TEST(DateFunctionsTest, HourColumn) {
    auto query = select_from<Timestamps>(hour("datetime_field"_col));
...
TEST(DateFunctionsTest, HourInWhere) {
    auto query = select_from<Timestamps>()
        | where(hour("datetime_field"_col) >= 9_v && hour("datetime_field"_col) <= 17_v);
...
TEST(DateFunctionsTest, MinuteColumn) {
    auto query = select_from<Timestamps>(minute("datetime_field"_col));
...
TEST(DateFunctionsTest, MinuteInWhere) {
    auto query = select_from<Timestamps>()
        | where(minute("datetime_field"_col) == 30_v);
...
TEST(DateFunctionsTest, SecondColumn) {
    auto query = select_from<Timestamps>(second("datetime_field"_col));
...
TEST(DateFunctionsTest, SecondInWhere) {
    auto query = select_from<Timestamps>()
        | where(second("datetime_field"_col) < 30_v);
...
TEST(DateFunctionsTest, WeekdayColumn) {
    auto query = select_from<Events>(weekday("event_date"_col));
...
TEST(DateFunctionsTest, WeekdayInWhere) {
    auto query = select_from<Events>()
        | where(weekday("event_date"_col) >= 1_v && weekday("event_date"_col) <= 5_v);
...
TEST(DateFunctionsTest, DaysBetweenColumns) {
    auto query = select_from<Events>(
        days_between("event_date"_col, "created_at"_col)
    );
...
TEST(DateFunctionsTest, DaysBetweenInWhere) {
    auto query = select_from<Events>()
        | where(days_between("event_date"_col, "created_at"_col) > 30_v);
...
TEST(DateFunctionsTest, DaysBetweenWithValue) {
    auto query = select_from<Events>()
        | where(days_between("event_date"_col, "2024-12-31"_v) <= 90_v);
...
TEST(DateFunctionsTest, UnixepochColumn) {
    auto query = select_from<Timestamps>(unixepoch("datetime_field"_col));
...
TEST(DateFunctionsTest, UnixepochInWhere) {
    auto query = select_from<Timestamps>()
        | where(unixepoch("datetime_field"_col) > 1700000000_v);
...
TEST(DateFunctionsTest, MultipleDateExtractors) {
    auto query = select_from<Events>(
        year("event_date"_col),
        month("event_date"_col),
        day("event_date"_col)
    );
...
TEST(DateFunctionsTest, DateFilterComplex) {
    auto query = select_from<Events>()
        | where(
            year("event_date"_col) == 2024_v &&
            month("event_date"_col) >= 6_v &&
            weekday("event_date"_col) != 0_v  // Not Sunday
        );
...
TEST(DateFunctionsTest, TimeRangeQuery) {
    auto query = select_from<Timestamps>()
        | where(
            hour("datetime_field"_col) >= 9_v &&
            hour("datetime_field"_col) < 17_v &&
            minute("datetime_field"_col) >= 0_v
        );
...
TEST(UtilityFunctionsTest, CastToInteger) {
    auto query = select_from<Events>(cast<int>("id"_col));
...
TEST(UtilityFunctionsTest, CastToReal) {
    auto query = select_from<Events>(cast<double>("id"_col));
...
TEST(UtilityFunctionsTest, CastToText) {
    auto query = select_from<Events>(cast<std::string>("id"_col));
...
TEST(UtilityFunctionsTest, CoalesceTwoColumns) {
    auto query = select_from<Events>(coalesce("name"_col, "Unknown"_v));
...
TEST(UtilityFunctionsTest, CoalesceMultipleValues) {
    auto query = select_from<Events>(
        coalesce("name"_col, "event_date"_col, "N/A"_v)
    );
...
TEST(UtilityFunctionsTest, CoalesceInWhere) {
    auto query = select_from<Events>()
        | where(coalesce("name"_col, ""_v) != ""_v);
...
TEST(UtilityFunctionsTest, CoalesceWithCast) {
    auto query = select_from<Events>(
        cast<std::string>(coalesce("id"_col, 0_v))
    );
